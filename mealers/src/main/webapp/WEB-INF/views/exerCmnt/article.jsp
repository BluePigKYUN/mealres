...
<body>
			...
			<div class="row g-4 content my-5 w-50" >
				<div class="border border-3 border-secondary rounded px-0 position-relative" style="height: auto; min-height: 800px">
					<div class="flex-grow-1" style="height: 50%">
						<img src="${pageContext.request.contextPath}/uploads/exerCmnt/${dto.fileName}" class="img-fluid rounded object-fit-cover" style="max-height: 460px">
					</div>
					<div class="flex-grow-1" style="height: 50%">
						 <div style="height: 75%">
				...
				<div class="col-lg-12 mt-4 px-0 d-flex justify-content-between">
					<div>
						<c:choose>
							<c:when test="${sessionScope.member.userNum == dto.userNum}">
								<button type="button" class="btn border border-secondary text-secondary rounded-pill" onclick="location.href='${pageContext.request.contextPath}/exerCmnt/update?num=${dto.num}&page=${page}&exerSort=${exerSort}';">수정</button>
							</c:when>
							<c:otherwise>
						...
						<c:choose>
							<c:when test="${sessionScope.member.userNum == dto.userNum || sessionScope.member.userId == 'admin'}">		
								<button type="button" class="btn border border-secondary text-secondary rounded-pill ms-1" onclick="deleteExer();">삭제</button>						
							</c:when>
							<c:otherwise>
								<button style="display: none"></button>
							</c:otherwise>
					...
					<div>
						<button type="button" class="btn text-white bg-secondary rounded-pill" onclick="location.href='${pageContext.request.contextPath}/exerCmnt/list?${query}';">목록</button>
					</div>
				</div>
			</div>
	...
	$(function() {
		...
		$(".likeheart").click(function() {
			...
			let url = "${pageContext.request.contextPath}/exerCmnt/addlikeCmnt";
			let num  = "${dto.num}";
	...
	function listPage(page) {
		let url = "${pageContext.request.contextPath}/exerCmnt/replyList";
	...
	$(function() {
		$(".replySubmit").click(function() {
			...
			let url = "${pageContext.request.contextPath}/exerCmnt/addReply";
	...
	$(function() {
		$(".reply").on("click", ".replyDelBtn", function() {
			...
			let url = "${pageContext.request.contextPath}/exerCmnt/removeReply";
		...
		<script type="text/javascript">
			function deleteExer() {
				if(confirm("글을 삭제하시겠습니까?")) {
					...
					let url = "${pageContext.request.contextPath}/exerCmnt/delete?" + query;
					location.href = url;
				}
			}
		</script>
