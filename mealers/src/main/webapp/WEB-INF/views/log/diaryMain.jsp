	...
	<div class="modal fade" id="diaryForm" data-bs-backdrop="static" data-bs-keyboard="false" 
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="diaryFormLabel">일기 보기</h5>
					<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
				</div>
				<div class="modal-body pt-0 pb-0">
	        		<form name="diaryForm">
	        			<table class="table">
							...
							<tr>
								<td class="col-2">제 목</td>
								<td>
									<input type="text" name="subject" id="form-subject" class="form-control" readonly>${dto.subject}
								</td>
							</tr>
	...
	//출력할 div 공간
	const listNode = document.querySelector('#printList');
	
	function loadContent(page) {
		//한개의 새로운 글을 불러올 때 사용할 속성
		//let num = "${dto.diary_Num}";
		let url = "${pageContext.request.contextPath}/log/diary";
	...
	function addNewContent(data) {
		...
		const itemCount = document.querySelector('.diaryCount');
		...
		diaryCount.innerHTML = '작성한 일기 ' + dataCount + '개';
		...
		for(let item of data.list){
			let reg_date_diary = item.reg_date_diary;
			let date = new Date(reg_date_diary);
			let fmt_date = date.getFullYear() + '-' + String(date.getMonth() + 1).padStart(2, '0') + '-' + String(date.getDate()).padStart(2, '0');
			let subject = item.subject;
			
			htmlText = '<div class="list-group-item d-flex align-items-start">';
			htmlText += '	<div class="me-3">';
			htmlText += '		<i class="bi bi-calendar3"></i>';
			htmlText += '	</div>';
			htmlText += '	<div>';
			htmlText += '		<span class="fw-bold d-block">' +  fmt_date + '</span>';
			htmlText += '		<p class="mb-0 d-block">' + subject + '</p>';
			htmlText += '	</div>';
			htmlText += '	<button type="button" class="btn ms-auto">일기 보기</button>';
			htmlText += '</div>';
			
			listNode.insertAdjacentHTML('afterend', htmlText);
		}
	}
