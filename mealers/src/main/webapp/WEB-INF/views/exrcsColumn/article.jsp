						...
						<td class="text-start">
								<c:choose>
									<c:when test="${sessionScope.member.userNum=='1'}">
										<button type="button" class="btn btn-light" onclick="location.href='${pageContext.request.contextPath}/exrcsColumn/update?num=${dto.num}&page=${page}&size=${size}';">수정</button>
									</c:when>
									<c:otherwise>
		...
		function deleteExrcsColumn() {
			if(confirm("게시글을 삭제 하시 겠습니까 ? ")) {
				let q = '${query}&num=${dto.num}';
				   let url = "${pageContext.request.contextPath}/exrcsColumn/delete";
				   location.href = url + '?' + q;
