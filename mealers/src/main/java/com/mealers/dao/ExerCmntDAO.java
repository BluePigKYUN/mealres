...
public class ExerCmntDAO {
	private Connection conn = DBConn.getConnection();
	
	public void insertExer(CmntDTO dto) throws SQLException {
		PreparedStatement pstmt = null;
		String sql;
		
		try {
			sql = "INSERT INTO exerCmnt(num, userNum, subject, content, reg_date, hitCount, fileName) "
					+ " VALUES (exerCmnt_seq.NEXTVAL, ?, ?, ?, SYSDATE, 0, ?)";
	...
	public int dataCount() {
		...
		try {
			sql = "SELECT NVL(COUNT(*),0) FROM exerCmnt";
			pstmt = conn.prepareStatement(sql);
	...
	public List<CmntDTO> listExer(int offset, int size, String exerSort) {
	...
	public List<CmntDTO> listExer(int offset, int size, String exerSort, String schCategory, String schContent) {
		List<CmntDTO> list = new ArrayList<CmntDTO>();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		StringBuilder sb = new StringBuilder();
	...
	public void hitCountCal(long num) throws SQLException {
		...
		try {
			sql = "UPDATE exerCmnt SET hitCount = hitCount+1 WHERE num = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setLong(1, num);
	...
	public boolean isLikeCmnt(long num, String userNum) {
		...
		try {
			sql = "SELECT num, userNum FROM exerCmntLike WHERE num = ? AND userNum = ?";
			pstmt = conn.prepareStatement(sql);
	...
	public void addlikeCmnt(long num, String userNum) throws SQLException {
		...
		try {
			sql = "INSERT INTO exerCmntLike(num, userNum) VALUES(?, ?)";
	...
	public void deleteLikeCmnt(long num, String userNum) throws SQLException {
		...
		try {
			sql = "DELETE FROM exerCmntLike WHERE num = ? AND userNum = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setLong(1, num);
			pstmt.setString(2, userNum);
	...
	public int likeCount(long num) {
		...
		try {
			...
			sql = "SELECT NVL(COUNT(*), 0) FROM exerCmntLike WHERE num = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setLong(1, num);
	...
	public void updateExer(CmntDTO dto) throws SQLException {
		PreparedStatement pstmt = null;
		String sql;
		
		try {
			sql = "UPDATE exerCmnt SET subject=?, content=?, fileName=? "
					+ " WHERE num = ?";
			pstmt = conn.prepareStatement(sql);
	...
	public void deleteExer(long num) throws SQLException {
		PreparedStatement pstmt = null;
		String sql;
		
		try {
			sql = "DELETE FROM exerCmnt WHERE num = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setLong(1, num);
	...
	public void addReply(ReplyDTO dto) throws SQLException {
		...
		try {
			sql = "INSERT INTO exerCmntReply(replyNum, num, userNum, content, reg_date) "
					+ " VALUES(exerCmntReply_seq.NEXTVAL, ?, ?, ?, SYSDATE)";
	...
	public List<ReplyDTO> replyList(long num, int offset, int size) {
		...
		try {
			...
			sb.append(" FROM exerCmntReply c ");
			sb.append(" JOIN member m ON c.userNum = m.userNum ");
			sb.append(" WHERE num = ? ");
	...
	public void removeReply(long replyNum, String userNum) throws SQLException {
		...
		try {
			sql = "DELETE FROM exerCmntReply WHERE replyNum = ?";
			pstmt = conn.prepareStatement(sql);
