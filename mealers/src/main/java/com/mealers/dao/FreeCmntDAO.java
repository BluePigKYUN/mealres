...
public class FreeCmntDAO {
	...
	public void insertFree(CmntDTO dto) throws SQLException {
		PreparedStatement pstmt = null;
		String sql;
		
		try {
			sql = "INSERT INTO freeCmnt(num, userNum, subject, content, reg_date, hitCount, fileName) "
					+ " VALUES (freeCmnt_seq.NEXTVAL, ?, ?, ?, SYSDATE, 0, ?)";
	...
	public int dataCount() {
		...
		try {
			sql = "SELECT NVL(COUNT(*),0) FROM freeCmnt";
			pstmt = conn.prepareStatement(sql);
	...
	public List<CmntDTO> listFree(int offset, int size, String freeSort) {
	...
	public List<CmntDTO> listFree(int offset, int size, String freeSort, String schCategory, String schContent) {
		List<CmntDTO> list = new ArrayList<CmntDTO>();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		StringBuilder sb = new StringBuilder();
	...
	public void hitCountCal(long num) throws SQLException {
		...
		try {
			sql = "UPDATE freeCmnt SET hitCount = hitCount+1 WHERE num = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setLong(1, num);
	...
	public boolean isLikeCmnt(long num, String userNum) {
		...
		try {
			sql = "SELECT num, userNum FROM freeCmntLike WHERE num = ? AND userNum = ?";
			pstmt = conn.prepareStatement(sql);
	...
	public void addlikeCmnt(long num, String userNum) throws SQLException {
		...
		try {
			sql = "INSERT INTO freeCmntLike(num, userNum) VALUES(?, ?)";
	...
	public void deleteLikeCmnt(long num, String userNum) throws SQLException {
		...
		try {
			sql = "DELETE FROM freeCmntLike WHERE num = ? AND userNum = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setLong(1, num);
			pstmt.setString(2, userNum);
	...
	public int likeCount(long num) {
		...
		try {
			...
			sql = "SELECT NVL(COUNT(*), 0) FROM freeCmntLike WHERE num = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setLong(1, num);
	...
	public void updateFree(CmntDTO dto) throws SQLException {
		PreparedStatement pstmt = null;
		String sql;
		
		try {
			sql = "UPDATE freeCmnt SET subject=?, content=?, fileName=? "
					+ " WHERE num = ?";
			pstmt = conn.prepareStatement(sql);
	...
	public void deleteFree(long num) throws SQLException {
		PreparedStatement pstmt = null;
		String sql;
		
		try {
			sql = "DELETE FROM freeCmnt WHERE num = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setLong(1, num);
	...
	public void addReply(ReplyDTO dto) throws SQLException {
		...
		try {
			sql = "INSERT INTO freeCmntReply(replyNum, num, userNum, content, reg_date) "
					+ " VALUES(freeCmntReply_seq.NEXTVAL, ?, ?, ?, SYSDATE)";
	...
	public List<ReplyDTO> replyList(long num, int offset, int size) {
		...
		try {
			...
			sb.append(" FROM freeCmntReply c ");
			sb.append(" JOIN member m ON c.userNum = m.userNum ");
			sb.append(" WHERE num = ? ");
	...
	public void removeReply(long replyNum, String userNum) throws SQLException {
		...
		try {
			sql = "DELETE FROM freeCmntReply WHERE replyNum = ?";
			pstmt = conn.prepareStatement(sql);
