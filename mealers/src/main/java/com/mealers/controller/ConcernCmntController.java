...
import com.mealers.dao.ConcernCmntDAO;
...
public class ConcernCmntController {
	...
	public ModelAndView list(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			String concernSort = req.getParameter("concernSort");
			if(concernSort == null) {
				concernSort = "recent";
			}
			...
			if(schContent.length() == 0) {
				list = dao.listConcern(offset, size, concernSort);
			} else {
				list = dao.listConcern(offset, size, concernSort, schCategory, schContent);
			}
			...
			String query = "concernSort=" + concernSort ;
			if(schContent.length() != 0) {
				query += "&schCategory=" + schCategory + "&schContent=" + URLEncoder.encode(schContent, "utf-8");
			}
			
			String cp = req.getContextPath();
			String listUrl = cp + "/concernCmnt/list?" + query;
			String articleUrl = cp + "/concernCmnt/article?page=" + current_page + "&" + query;
			...
			mav.addObject("concernSort", concernSort);
	...
	@RequestMapping(value = "/concernCmnt/write", method = RequestMethod.GET)
	public ModelAndView write(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		ModelAndView mav = new ModelAndView("concernCmnt/write");
	...
	@RequestMapping(value = "/concernCmnt/write", method = RequestMethod.POST)
	public ModelAndView writeOk(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String pathname = root + "uploads" + File.separator + "concernCmnt";
		...
		try {
			...
			if(filename != null) {
				...
				dao.insertConcern(dto);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		
		return new ModelAndView("redirect:/concernCmnt/list");
	}
	
	@RequestMapping(value = "/concernCmnt/article", method = RequestMethod.GET)
	public ModelAndView article(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		// 페이지, 글번호, [,검색컬럼, 검색값, 정렬컬럼]
		ConcernCmntDAO dao = new ConcernCmntDAO();
		String page = req.getParameter("page");
		String concernSort = req.getParameter("concernSort");
		String query = "page=" + page + "&concernSort=" + concernSort;
		...
		try {
			...
			if(dto == null) {
				return new ModelAndView("/concernCmnt/list?" + query);
			}
			...
			ModelAndView mav = new ModelAndView("concernCmnt/article");
	...
	@ResponseBody
	@RequestMapping(value = "/concernCmnt/addlikeCmnt", method = RequestMethod.POST)
	...
	@RequestMapping(value = "/concernCmnt/update", method = RequestMethod.GET)
	public ModelAndView updateConcern(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if(dto == null || ! dto.getUserNum().equals(info.getUserNum())) {
				return new ModelAndView("redirect:/concernCmnt/list?page="+ page + "&concernSort=" + concernSort);
			}
	...
	@RequestMapping(value = "/concernCmnt/update", method = RequestMethod.POST)
	public ModelAndView updateConcernOk(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
	...
	@RequestMapping(value = "/concernCmnt/delete", method = RequestMethod.GET)
	public ModelAndView deleteConcern(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		// 페이지, 글번호, 정렬
		ConcernCmntDAO dao = new ConcernCmntDAO();
		...
		try {
			...
			if(dto == null) {
				return new ModelAndView("redirect:/concernCmnt/list?" + query);
			}
	...
	@ResponseBody
	@RequestMapping(value = "/concernCmnt/addReply", method = RequestMethod.POST)
	...
	@RequestMapping(value = "/concernCmnt/replyList", method = RequestMethod.GET)
	...
	@ResponseBody
	@RequestMapping(value = "/concernCmnt/removeReply", method = RequestMethod.POST)
	...
	@ResponseBody
	@RequestMapping(value = "/concernCmnt/adoptReply", method = RequestMethod.POST)
	public Map<String, Object> adoptReply(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		Map<String, Object> model = new HashMap<String, Object>();
